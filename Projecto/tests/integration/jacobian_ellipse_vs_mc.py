"""
VALIDATION_integration_jacobian_ellipse_vs_mc_SPEC.py
=============================================
...
Run: `python integration_phase_SPEC/VALIDATION_integration_jacobian_ellipse_vs_mc_SPEC.py`
"""
...
# Prior 3-DOF tuned knots extended with mild shoulder-yaw spline (deg).
_TUNED_KNOTS12 = np.radians(
    np.array(
        [
            ...
            -37.46583288056429,
            -33.196898815597365,
            8.519418496181338,  # shoulder
            -111.8152288749504,
            -76.08164184800785,
            -17.940545379302467,  # elbow
            -2.162963251122143,
            17.39559530401733,
            20.99761664028883,  # wrist
        ],
        dtype=float,
    )
)
...
def jacobian_comparison_SPEC(n_mc: int = 80, seed: int = 42):
    ...
    release_states, landings, scores, hit_mask = run_mc_and_collect_states(
        _TUNED_KNOTS12, n=n_mc, seed=seed)
    ...
    out_path = _ARTIFACTS / "integration_jacobian_ellipse_vs_mc_SPEC.png"
...
def optimizer_SPEC(n_mc_per_eval: int = 30, maxiter: int = 40, seed: int = 7):
    """
    Nelder-Mead on the 12 spline knots, maximising expected score under torque noise.
    Prints before/after mean scores.
    """
    print("--- C2 Nelder-Mead optimizer ---")

    def mc_objective(knots12):
        rng_inner = np.random.default_rng(seed)
        _, _, sc, _ = run_mc_and_collect_states(knots12, n=n_mc_per_eval, seed=rng_inner.integers(1 << 31))
        return float(np.mean(sc)) if len(sc) > 0 else 0.0

    x0 = _TUNED_KNOTS12.copy()
