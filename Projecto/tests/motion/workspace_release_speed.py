"""
VALIDATION_motion_workspace_release_speed_SPEC.py
==============================================
...
Run: `pytest tests/motion/workspace_release_speed.py -q` (or `dartrobot demo`).
"""
...
from dartrobot.motion.controller import (
    ...
    release_chain_positions_world_SPEC,
    release_site_fk_SPEC,
    simulate_throw_mujoco_SPEC,
)

# A4 visualization uses the tall **4-DOF** model.
_XML_PATH = mjcf_path_SPEC("arm_4dof_tall.xml")

_ARTIFACTS = artifacts_dir_SPEC("motion")
...
def _arm_segment_xz(q4: np.ndarray, shoulder_mount_z_m: float) -> np.ndarray:
    """Return (4, 2) array of [shoulder, elbow, wrist_joint, tip] in the x-z plane (projection)."""
    q4 = np.asarray(q4, dtype=float).reshape(4)
    o3, e3, w3, t3 = release_chain_positions_world_SPEC(q4)
    # Use model mount height for the first point so the stick figure lines up with MJCF mount z.
    o = np.array([SHOULDER_MOUNT_X_M, shoulder_mount_z_m], dtype=float)
    elbow = np.array([e3[0], e3[2]], dtype=float)
    wrist = np.array([w3[0], w3[2]], dtype=float)
    tip = np.array([t3[0], t3[2]], dtype=float)
    return np.array([o, elbow, wrist, tip])


def workspace_grid_SPEC(n_each=4):
    ...
    for q0 in qs[0]:
        for q1 in qs[1]:
            for q2 in qs[2]:
                for q3 in qs[3]:
                    p, _ = release_site_fk_SPEC(np.array([q0, q1, q2, q3]))
                    max_x = max(max_x, p[0])
                    max_z = max(max_z, p[2])
    ...
    return {"max_x_m": float(max_x), "max_z_m": float(max_z)}
...
def sample_throw_speed_SPEC() -> dict:
    ...
    print(
        "  actuator saturation ratio (yaw/shoulder/elbow/wrist): "
        f"{sat_ratio[0]:.2%}, {sat_ratio[1]:.2%}, {sat_ratio[2]:.2%}, {sat_ratio[3]:.2%}"
    )
    out["release_speed_mps"] = spd
    out["sat_ratio"] = sat_ratio
    return out
...
def plot_arm_throw_SPEC(sim_result: dict, n_frames: int = 6, save: bool = True):
    ...
    out_path = _ARTIFACTS / "motion_throw_stick_figure_SPEC.png"
    if save:
...
def render_throw_video_SPEC(
    ...
    """
    if out_path is None:
        out_path = _ARTIFACTS / "motion_throw_video_SPEC.mp4"

    q_traj = sim_result["q"]          # (T, 4) joint angles
    ...
    print(f"Saved: {out_path}")
    print(f"  Video stats: {len(frames)} frames at {fps} fps")
    return out_path


def _write_manifest_SPEC(workspace: dict, sim_result: dict, stick_path: Path, video_path: Path) -> Path:
    s6 = np.asarray(sim_result.get("release_state6"), dtype=float).reshape(6)
    release_speed = float(np.linalg.norm(s6[3:6]))
    manifest = {
        "stage": "motion_workspace_and_throw",
        "metrics": {
            "workspace_max_x_m": float(workspace["max_x_m"]),
            "workspace_max_z_m": float(workspace["max_z_m"]),
            "release_speed_mps": release_speed,
            "release_state6": s6.tolist(),
            "sat_ratio": np.asarray(sim_result.get("sat_ratio", np.zeros(4)), dtype=float).tolist(),
        },
        "artifacts": [
            {
                "path": str(stick_path),
                "caption": "A4 stick-figure throw with release/board annotations.",
            },
            {
                "path": str(video_path),
                "caption": "A4 MuJoCo replay video of the throwing motion.",
            },
        ],
    }
    manifest_path = _ARTIFACTS / "motion_demo_manifest_SPEC.json"
...
if __name__ == "__main__":
    ws = workspace_grid_SPEC()
    result = sample_throw_speed_SPEC()
    print("--- A4 stick-figure render ---")
    stick = plot_arm_throw_SPEC(result)
    print("--- A4 video render ---")
    video = render_throw_video_SPEC(result)
    _write_manifest_SPEC(ws, result, stick, video)
